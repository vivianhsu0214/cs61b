...
class Machine {
    ...
    void insertRotors(String[] rotors) {
        if(rotors.length > _slots.length) {
            System.arraycopy(rotors, 0, rotors, 0, _slots.length);
        }
        for (int i = 0; i < rotors.length; i++) {
            String key = rotors[i];
            if(_archive.containsKey(key)) {
                if(i == 0 && !_archive.get(key).reflecting()) {
                    throw new EnigmaException(String.format("The first rotor is not a reflector!", key));
                }
                if(i < _slots.length - _numPawls && _archive.get(key).rotates()) {
                    throw new EnigmaException(String.format("Some rotors are expected to be fixed!", key));
                }
    ...
    void setRotors(String setting) {
        if(setting.length() != _slots.length - 1) {
            throw new EnigmaException(String.format("Size of setting info is not correct!", setting, setting.length()));
        }
    ...
    String convert(String msg) {
        ...
        for(int i = 0; i < msg.length(); i += 1) {
            buffer[i] = _alphabet.toChar(convert(_alphabet.toInt(msg.charAt(i))));
        }
